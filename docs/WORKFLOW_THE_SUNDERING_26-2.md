...
- **Fase 0 completada para LionfishAPI/NautilusAPI Y para Cataclysm, incluyendo los 3 bloqueos reales ya resueltos** — ver `temp/cataclysm_port_fase0/FASE0_REPORT.md` (LionfishAPI) y `temp/cataclysm_port_fase0/FASE0_REPORT_CATACLYSM.md` (Cataclysm, 2026-08-15) fuera de este repo, en la raíz del workspace de mods. **Fase 1 ya planificada** (2026-08-15, antes de empezar a ejecutarla) en `temp/cataclysm_port_fase0/FASE1_PLAN_CATACLYSM.md` — pasada mecánica global primero (ResourceLocation→Identifier, renombres de paquete, etc.), luego 19 bloques ordenados por dependencia (foundation → registries → infraestructura de render → lógica de entidades por facción), mixins/AT al final. Leer ese plan antes de empezar cualquier trabajo de Fase 1 — no re-planificar desde cero. El diagnóstico real de Cataclysm dio **10.559 errores de compilación** sobre 839 clases (con NautilusAPI ya en el classpath para no mezclar ruido). La mayoría son fallout mecánico de patrones ya confirmados (ResourceLocation→Identifier ~1900 hits, Level.isClientSide 418 hits, RenderType reubicado, animate()/AnimationState ya resuelto en NautilusAPI).
