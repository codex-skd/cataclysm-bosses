# Flujo de trabajo — Cataclysm Bosses (NeoForge)

> **Versión del workflow**: 1.16.0 (codex-docs)
> Este archivo pertenece al proyecto **Cataclysm Bosses**. Cambios aquí solo afectan a este proyecto.
>
> ⚠️ **Renombrado en curso — "The Sundering" → "Cataclysm Bosses"** (2026-08-29). El código y el build ya usan `mod_id=cataclysm_bosses` / package `com.skd.cataclysmbosses` / clase `@Mod` `Cataclysm_Bosses`. Este documento conserva pasajes históricos que aún dicen `the_sundering` / `com.skd.thesundering` / "decisión del usuario 2026-08-18 de usar el nombre completo": esa decisión quedó **superada** por el renombrado a `cataclysm_bosses`. Estado real de compilación: `docs/PORT_STATUS_26-2.md`.
...
| Dato | Valor |
|---|---|
| Mod ID (`gradle.properties`) | `cataclysm_bosses` |
| Package Java | `com.skd.cataclysmbosses` |
| Clase principal (`@Mod`) | `Cataclysm_Bosses` |
| Namespace de recursos | `cataclysm` (interno, distinto del `mod_id` — no "corregir") |
| Display name (Title Case) | `Cataclysm Bosses` |
