...
public class ItemStackTheFlatteningFix extends DataFix {
    ...
    private static final Set<String> IDS = MAP.keySet().stream().map(k -> k.substring(0, k.indexOf(46))).collect(Collectors.toSet());
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", itemStackType, input -> {
            ...
            String newValue = updateItem(id.get().getSecond(), data);
            if (newValue != null) {
                output = output.set(idF, Pair.of(References.ITEM_NAME.typeName(), newValue));
            }

            if (DAMAGE_IDS.contains(id.get().getSecond())) {
            ...
            return output.set(DSL.remainderFinder(), rest.remove("Damage"));
        });
    }

    public static @Nullable String updateItem(@Nullable String name, int data) {
        if (IDS.contains(name)) {
            String newName = MAP.get(name + "." + data);
            return newName == null ? MAP.get(name + ".0") : newName;
        } else {
            return null;
        }
    }
