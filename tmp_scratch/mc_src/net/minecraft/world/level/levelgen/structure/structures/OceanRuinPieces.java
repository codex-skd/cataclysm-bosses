    ...
    public static class OceanRuinPiece extends TemplateStructurePiece {
        ...
        protected void handleDataMarker(String markerId, BlockPos position, ServerLevelAccessor level, RandomSource random, BoundingBox chunkBB) {
            if ("chest".equals(markerId)) {
                level.setBlock(
                    position, Blocks.CHEST.defaultBlockState().setValue(ChestBlock.WATERLOGGED, level.getFluidState(position).is(FluidTags.WATER)), 2
                );
                if (level.getBlockEntity(position) instanceof ChestBlockEntity chestBlockEntity) {
                    chestBlockEntity.setLootTable(
                        this.isLarge ? BuiltInLootTables.UNDERWATER_RUIN_BIG : BuiltInLootTables.UNDERWATER_RUIN_SMALL, random.nextLong()
                    );
                }
