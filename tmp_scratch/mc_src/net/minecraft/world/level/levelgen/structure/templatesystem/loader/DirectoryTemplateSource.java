...
public class DirectoryTemplateSource extends TemplateSource {
    ...
    public DirectoryTemplateSource(
        DataFixer fixerUpper, HolderGetter<Block> blockLookup, Path sourceDir, PackType packType, FileToIdConverter fileToIdConverter, boolean loadAsText
    ) {
        this(fixerUpper, blockLookup, sourceDir.resolve(packType.getDirectory()), fileToIdConverter, loadAsText);
    }

    public DirectoryTemplateSource(
        DataFixer fixerUpper, HolderGetter<Block> blockLookup, Path sourceDir, FileToIdConverter fileToIdConverter, boolean loadAsText
    ) {
