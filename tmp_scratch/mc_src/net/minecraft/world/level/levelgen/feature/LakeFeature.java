        ...
        if (fluid.getFluidState().is(FluidTags.WATER)) {
            for (int xx = 0; xx < 16; xx++) {
                for (int zz = 0; zz < 16; zz++) {
                    ...
                    if (level.getBiome(offset).value().shouldFreeze(level, offset, false) && config.canReplaceWithAirOrFluid.test(level, offset)) {
    ...
    public record Configuration(
        ...
        public static final Codec<LakeFeature.Configuration> CODEC = RecordCodecBuilder.create(
            i -> i.group(
                    ...
                    BlockPredicate.CODEC.fieldOf("can_place_feature").forGetter(LakeFeature.Configuration::canPlaceFeature),
                    BlockPredicate.CODEC.fieldOf("can_replace_with_air_or_fluid").forGetter(LakeFeature.Configuration::canReplaceWithAirOrFluid),
                    BlockPredicate.CODEC.fieldOf("can_replace_with_barrier").forGetter(LakeFeature.Configuration::canReplaceWithBarrier)
                )
