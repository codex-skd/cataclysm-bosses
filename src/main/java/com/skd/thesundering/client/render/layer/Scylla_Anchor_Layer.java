...
extends RenderLayer<Scylla_Entity, Scylla_Model> {
    ...
    private void drawLightningAnchor(PoseStack matrixStackIn, MultiBufferSource bufferIn, Scylla_Entity entity, float xyz, int r, int g, int b, int seg, float size, float partialTicks) {
        matrixStackIn.pushPose();
        double x = (this.rnd.nextFloat() - 0.5f) * xyz;
        double y = (this.rnd.nextFloat() - 0.5f) * xyz;
        double z = (this.rnd.nextFloat() - 0.5f) * xyz;
        ...
        LightningBoltData bolt1 = new LightningBoltData(blueBoltData, Vec3.ZERO, new Vec3(x, y, z), seg).size(size).lifespan(1).spawn(LightningBoltData.SpawnFunction.NO_DELAY).fade(LightningBoltData.FadeFunction.NONE);
    ...
    public Vec3 getAnchorPosition(Vec3 offsetIn) {
        ...
        ((Scylla_Model)this.getParentModel()).translateChainAnchor(translationStack);
