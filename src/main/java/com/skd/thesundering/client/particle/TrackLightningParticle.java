...
extends Particle {
    ...
    public TrackLightningParticle(ClientLevel world, double x, double y, double z, double xd, double yd, double zd, int r, int g, int b) {
        ...
        LightningBoltData bolt = new LightningBoltData(boltData, Vec3.ZERO, lightningTo, sections).size(0.1f + this.random.nextFloat() * 0.1f).lifespan(this.lifetime).spawn(LightningBoltData.SpawnFunction.CONSECUTIVE);
