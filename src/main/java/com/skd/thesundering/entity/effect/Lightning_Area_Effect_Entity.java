...
public class Lightning_Area_Effect_Entity
extends Entity {
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(Lightning_Area_Effect_Entity.class, (EntityDataSerializer)EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> DAMAGE = SynchedEntityData.defineId(Lightning_Area_Effect_Entity.class, (EntityDataSerializer)EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Boolean> DATA_WAITING = SynchedEntityData.defineId(Lightning_Area_Effect_Entity.class, (EntityDataSerializer)EntityDataSerializers.BOOLEAN);
    ...
    public Lightning_Area_Effect_Entity(Level p_19707_, double p_19708_, double p_19709_, double p_19710_) {
        this((EntityType<? extends Lightning_Area_Effect_Entity>)((EntityType)ModEntities.LIGHTNING_AREA_EFFECT.get()), p_19707_);
    ...
    public void setDamage(float f) {
        this.entityData.set(DAMAGE, (Object)Float.valueOf(f));
    }
    ...
    public void tick() {
        ...
        if (this.level().isClientSide()) {
            ...
            for (int j = 0; j < 2 + this.random.nextInt(1); ++j) {
                ...
                this.level().addAlwaysVisibleParticle((ParticleOptions)new LightningZapParticleOptions(99, 194, 201, 0.0f), d0, d2, d4, 0.0, this.random.nextGaussian() * 0.07, 0.0);
            }
            if (this.tickCount % 10 == 0) {
                this.level().addAlwaysVisibleParticle((ParticleOptions)new RainFogParticleOptions(f), this.getX(), this.getY() + 0.01, this.getZ(), this.random.nextGaussian() * 0.01, 0.0, this.random.nextGaussian() * 0.01);
            }
        } else {
            boolean flag1;
    ...
    protected void damage(LivingEntity Hitentity) {
        ...
        if (Hitentity.isAlive() && !Hitentity.isInvulnerable() && Hitentity != caster && this.tickCount % 5 == 0) {
            ...
            } else if (!caster.isAlliedTo((Entity)Hitentity) && !Hitentity.isAlliedTo((Entity)caster)) {
                Hitentity.hurt(CMDamageTypes.causeLightningDamage(this, (Entity)caster), this.getDamage());
            }
        }
    }
