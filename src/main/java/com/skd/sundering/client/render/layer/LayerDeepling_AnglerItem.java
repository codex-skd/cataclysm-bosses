...
public class LayerDeepling_AnglerItem
extends RenderLayer<Deepling_Angler_Entity, Deepling_Angler_Model> {
    ...
    protected void translateToHand(PoseStack matrixStack, boolean left) {
        ((Deepling_Angler_Model)this.getParentModel()).root.translateAndRotate(matrixStack);
        ((Deepling_Angler_Model)this.getParentModel()).body.translateAndRotate(matrixStack);
        if (left) {
            ((Deepling_Angler_Model)this.getParentModel()).left_arm.translateAndRotate(matrixStack);
        } else {
            ((Deepling_Angler_Model)this.getParentModel()).right_arm.translateAndRotate(matrixStack);
        }
    }
}
