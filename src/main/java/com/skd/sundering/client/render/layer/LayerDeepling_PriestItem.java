...
import com.skd.sundering.client.model.entity.Deepling_Priest_Model;
...
public class LayerDeepling_PriestItem
extends RenderLayer<Deepling_Priest_Entity, Deepling_Priest_Model> {
    ...
    protected void translateToHand(PoseStack matrixStack, boolean left) {
        ...
        if (left) {
            ((Deepling_Priest_Model)this.getParentModel()).left_arm.translateAndRotate(matrixStack);
        } else {
            ((Deepling_Priest_Model)this.getParentModel()).right_arm.translateAndRotate(matrixStack);
        }
    }
}
