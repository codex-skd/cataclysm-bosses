...
extends SingleQuadParticle {
    ...
    public Circle_Particle(ClientLevel world, double x, double y, double z, double motionX, double motionY, double motionZ, float yaw, float pitch, int duration, int r, int g, int b, float opacity, float opacitystart, float size, boolean facesCamera, int behavior, SpriteSet sprites) {
        super(world, x, y, z);
    ...
    public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
        ...
        float smoothAge = (float)this.age + partialTicks;
        float fadeProgress = (smoothAge - this.opacitystart) / ((float)this.lifetime - this.opacitystart);
        float fadeMultiplier = 1.0f - fadeProgress;
        fadeMultiplier = Mth.clamp((float)fadeMultiplier, (float)0.0f, (float)1.0f);
        this.alpha = this.opacity * (0.95f * fadeMultiplier + 0.05f);
        this.rCol = this.r;
        this.gCol = this.g;
        this.bCol = this.b;
    ...
    implements ParticleProvider<Circle_ParticleOptions> {
        private final SpriteSet spriteSet;

        public CircleFactory(SpriteSet sprite) {
            this.spriteSet = sprite;
        }

        public Particle createParticle(Circle_ParticleOptions typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
