...
public record RingParticleOptions(float yaw, float pitch, int duration, int r, int g, int b, float a, float scale, boolean facesCamera, int behavior) implements ParticleOptions
{
    ...
    public static MapCodec<RingParticleOptions> MAP_CODEC = RecordCodecBuilder.mapCodec(object -> object.group(Codec.FLOAT.fieldOf("yaw").forGetter(p -> Float.valueOf(p.yaw)), Codec.FLOAT.fieldOf("pitch").forGetter(p -> Float.valueOf(p.pitch)), Codec.INT.fieldOf("duration").forGetter(p -> p.duration), Codec.INT.fieldOf("r").forGetter(p -> p.r), Codec.INT.fieldOf("g").forGetter(p -> p.g), Codec.INT.fieldOf("b").forGetter(p -> p.b), Codec.FLOAT.fieldOf("a").forGetter(p -> Float.valueOf(p.a)), Codec.FLOAT.fieldOf("scale").forGetter(p -> Float.valueOf(p.scale)), Codec.BOOL.fieldOf("facescamera").forGetter(p -> p.facesCamera), Codec.INT.fieldOf("behavior").forGetter(p -> p.behavior)).apply(object, RingParticleOptions::new));
