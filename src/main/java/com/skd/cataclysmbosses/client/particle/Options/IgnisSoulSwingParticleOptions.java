...
public record IgnisSoulSwingParticleOptions(int lifetime, float scale, float yaw, float pitch, float roll) implements ParticleOptions
{
    ...
    public static MapCodec<IgnisSoulSwingParticleOptions> MAP_CODEC = RecordCodecBuilder.mapCodec(object -> object.group(Codec.INT.fieldOf("lifetime").forGetter(p -> p.lifetime), Codec.FLOAT.fieldOf("scale").forGetter(p -> Float.valueOf(p.scale)), Codec.FLOAT.fieldOf("yaw").forGetter(p -> Float.valueOf(p.yaw)), Codec.FLOAT.fieldOf("pitch").forGetter(p -> Float.valueOf(p.pitch)), Codec.FLOAT.fieldOf("roll").forGetter(p -> Float.valueOf(p.roll))).apply(object, IgnisSoulSwingParticleOptions::new));
