...
public record NotSpinTrailParticleOptions(float r, float g, float b, float gravity, float reduction, float acceleration, double direction, int life) implements ParticleOptions
{
    ...
    public static MapCodec<NotSpinTrailParticleOptions> MAP_CODEC = RecordCodecBuilder.mapCodec(object -> object.group(Codec.FLOAT.fieldOf("r").forGetter(p -> Float.valueOf(p.r)), Codec.FLOAT.fieldOf("g").forGetter(p -> Float.valueOf(p.g)), Codec.FLOAT.fieldOf("b").forGetter(p -> Float.valueOf(p.b)), Codec.FLOAT.fieldOf("gravity").forGetter(p -> Float.valueOf(p.gravity)), Codec.FLOAT.fieldOf("reduction").forGetter(p -> Float.valueOf(p.reduction)), Codec.FLOAT.fieldOf("acceleration").forGetter(p -> Float.valueOf(p.acceleration)), Codec.DOUBLE.fieldOf("direction").forGetter(p -> p.direction), Codec.INT.fieldOf("life").forGetter(p -> p.life)).apply(object, NotSpinTrailParticleOptions::new));
